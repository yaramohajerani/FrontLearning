...
def unet_model_linear_normalized(height=0,width=0,channels=1,n_init=12,n_layers=2):
    ...
    for i in range(1,n_layers+1):
        #-- convolution layer
        c[i] = BatchNormalization()(kl.Conv2D(n_init*i,3,activation='relu',padding='same')(p[i-1]))
        c[i] = BatchNormalization()(kl.Conv2D(n_init*i,3,activation='relu',padding='same')(c[i]))
    ...
    while count>1:
        ...
        #-- where the missing elements in the 2nd layer are padded with 0
        #-- concatenating along the color channels
        upsampled_c[i] = BatchNormalization()(kl.UpSampling2D(size=(2,2))(c[count]))
        up[i] = kl.concatenate([upsampled_c[i],c[count-1]],axis=3)
        #-- now do a convolution with the merged upsampled layer
        i += 1
        c[i] = BatchNormalization()(kl.Conv2D(n_init*(count-1),3,activation='relu',padding='same')(up[i-1]))
        c[i] = BatchNormalization()(kl.Conv2D(n_init*(count-1),3,activation='relu',padding='same')(c[i]))
        #-- counter decreases as we go back up
        count -= 1
    ...
    #-- convlution across the last n_iniy filters into 3 channels
    i += 1
    c[i] = BatchNormalization()(kl.Conv2D(3,3,activation='relu',padding='same')(c[i-1]))
    #-- do one final sigmoid convolution into just 1 final channel (None,h,w,1)
    i += 1
    c[i] = BatchNormalization()(kl.Conv2D(1,1,activation='sigmoid')(c[i-1]))
    #-- reshape into a flattened output to match sample weights
    i += 1
    c[i] = kl.Reshape((height*width,1,))(c[i-1])
    print 'output shape: ', c[i].shape
...
def unet_model_double_normalized(height=0,width=0,channels=1,n_init=12,n_layers=2):
    ...
    for i in range(1,n_layers+1):
        #-- convolution layer
        c[i] = BatchNormalization()(kl.Conv2D(n_filts,3,activation='relu',padding='same')(p[i-1]))
        c[i] = BatchNormalization()(kl.Conv2D(n_filts,3,activation='relu',padding='same')(c[i]))
    ...
    while count>1:
        ...
        #-- now do a convlution with the merged upsampled layer
        i += 1
        c[i] = BatchNormalization()(kl.Conv2D(n_filts,3,activation='relu',padding='same')(up[i-1]))
        c[i] = BatchNormalization()(kl.Conv2D(n_filts,3,activation='relu',padding='same')(c[i]))
        #-- counter decreases as we go back up
        count -= 1
