...
outputFolder='/Users/mhwood/Documents/Research/Projects/Front Learning/Results/'+glacier+' Results/Histograms/'+method

labelFolder='/Users/mhwood/Documents/Research/Projects/Front Learning/Results/'+glacier+' Results/'+method+'/'+method

cnnPixelFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/' + glacier + ' Results/'+method+'/'+method+' Pixel CSVs'
sobelPixelFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/' + glacier + ' Results/Sobel/Sobel Pixel CSVs'

trueFrontFolder = '/Users/mhwood/Documents/Research/Projects/Melt Rate Calculation/Regions/' + coast + '/Glaciers/' + glacier + '/Retreat/Front Locations/3413'
cnnFrontFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/' + glacier + ' Results/'+method+'/'+method+' Geo CSVs'
sobelFrontFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/' + glacier + ' Results/Sobel/Sobel Geo CSVs'
...
def getFrontList(glacier,labelList):
    metadataFile='/Users/mhwood/Documents/Research/Projects/Front Learning/Glaciers/'+glacier+'/'+glacier+' Image Data.csv'
    f=open(metadataFile)
    lines=f.read()
    f.close()
    lines=lines.split('\n')
...
for ll in range(N):
    ...
    trueImageFolder='/Users/mhwood/Documents/Research/Projects/Front Learning/Glaciers/'+glacier+'/Small Images'
    trueImage = Image.open(trueImageFolder+'/'+label+'_Subset.png').transpose(Image.FLIP_LEFT_RIGHT).convert("L")

    cnnImageFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/'+glacier+' Results/'+method+'/'+method
    cnnImage = Image.open(cnnImageFolder + '/' + label + '_nothreshold.png').transpose(Image.FLIP_LEFT_RIGHT).convert("L")

    sobelImageFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/'+glacier+' Results/Sobel/Sobel'
    ...
    plt.gca().axes.get_xaxis().set_ticks([])
    plt.gca().axes.get_yaxis().set_ticks([])
    plt.title('Sobel Solution')

    plt.subplot(2,3,4)
    plt.title('Geocoded Solutions')
    ...
    plt.subplot(2,3,5)
    plt.title('NN Errors Histogram')
    bins=range(0,5000,100)
    y1, x1, _ =plt.hist(cnnErrors,alpha=0.5,color='blue',bins=bins,label='NN')
    plt.xlabel('RMS Error = '+'{0:.2f}'.format(rmsError(cnnErrors))+' m')

    plt.subplot(2, 3, 6)
    plt.title('Sobel Error Histogram')
    bins = range(0, 5000, 100)
    y2, x2, _ =plt.hist(sobelErrors, alpha=0.5, color='green', bins=bins, label='Sobel')
    plt.xlabel('RMS Error = ' + '{0:.2f}'.format(rmsError(sobelErrors)) + ' m')
...
plt.subplot(1,2,1)
plt.title('NN Error Histogram')
bins=range(0,5000,100)
y1, x1, _ =plt.hist(allCNNerrors,alpha=0.5,color='blue',bins=bins,label='NN')
plt.xlabel('RMS Error = '+'{0:.2f}'.format(rmsError(allCNNerrors))+' m')
plt.ylabel('Count (100 m bins)')

plt.subplot(1,2,2)
plt.title('Sobel Error Histogram')
bins = range(0, 5000, 100)
y2, x2, _ =plt.hist(allSobelerrors, alpha=0.5, color='green', bins=bins, label='Sobel')
plt.xlabel('RMS Error = ' + '{0:.2f}'.format(rmsError(allSobelerrors)) + ' m')
...
outputFolder = '/Users/mhwood/Documents/Research/Projects/Front Learning/Results/Helheim Results/'
